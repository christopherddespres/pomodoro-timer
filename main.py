from tkinter import *
import math

# ---------------------------- CONSTANTS ------------------------------- #
PINK = "#e2979c"
RED = "#e7305b"
GREEN = "#9bdeac"
YELLOW = "#f7f5dd"
FONT_NAME = "Courier"
WORK_MIN = 25
SHORT_BREAK_MIN = 5
LONG_BREAK_MIN = 20
reps = 0
timer = None

# ---------------------------- TIMER RESET ------------------------------- # 


def reset_timer():
    global timer
    global reps
    window.after_cancel(timer)
    canvas.itemconfig(timer_text, text="00:00")
    mode_label.config(text="")
    check_label.config(text="")
    reps = 0


# ---------------------------- TIMER MECHANISM ------------------------------- # 
def start_timer():
    global reps
    reps += 1
    work_sec = WORK_MIN * 60
    short_break_sec = SHORT_BREAK_MIN * 60
    long_break_sec = LONG_BREAK_MIN * 60

    if reps % 8 == 0:
        count_down(long_break_sec)
        mode_label.config(text="LONG BREAK", fg=RED)
    elif reps % 2 == 0:
        count_down(short_break_sec)
        mode_label.config(text="SHORT BREAK", fg=PINK)
    else:
        count_down(work_sec)
        mode_label.config(text="WORK TIME", fg=GREEN)


# ---------------------------- COUNTDOWN MECHANISM ------------------------------- # 
def count_down(count):

    count_min = math.floor(count / 60)
    count_sec = count % 60
    if count_sec < 10:
        count_sec = f"0{count_sec}"

    canvas.itemconfig(timer_text, text=f"{count_min}:{count_sec}")
    if count > 0:
        global timer
        timer = window.after(1000, count_down, count - 1)
    else:
        start_timer()
        marks = ""
        work_sessions = math.floor(reps/2)
        for _ in range(work_sessions):
            marks += "✓"
        check_label.config(text=marks)


# ---------------------------- UI SETUP ------------------------------- #
window = Tk()
window.title("Pomodoro")
window.config(padx=100, pady=50, bg=YELLOW)


canvas = Canvas(width=200, height=224, bg=YELLOW, highlightthickness=0)
tomato_img = PhotoImage(file="tomato.png")
canvas.create_image(100, 112, image=tomato_img)
timer_text = canvas.create_text(100, 130, text="00:00", fill="white", font=(FONT_NAME, 35, "bold"))
canvas.grid(column=1, row=2)


# Create labels
# title_label
title_label = Label(text="Pomodoro Timer", bg=YELLOW, font=(FONT_NAME, 30, "bold"))
title_label.grid(row=0, columnspan=3)
title_label.config(pady=2)

# mode_label
mode_label = Label(text="", bg=YELLOW, fg=GREEN, font=(FONT_NAME, 40, "bold"))
mode_label.grid(row=1, columnspan=3)
mode_label.config(pady=2)

# check_label
check_label = Label(bg=YELLOW, fg=GREEN, font=(FONT_NAME, 30, "bold"))
check_label.grid(row=3, column=1)
check_label.config(pady=2)

# Create buttons
# start_button
start_button = Button(text="Start", bg="white", highlightthickness=0, font=(FONT_NAME, 10), command=start_timer)
start_button.grid(row=3, column=0)
start_button.config(padx=3, pady=3)

# reset_button
reset_button = Button(text="Reset", bg="white", highlightthickness=0, font=(FONT_NAME, 10), command=reset_timer)
reset_button.grid(row=3, column=2)
reset_button.config(padx=3, pady=3)


window.mainloop()
